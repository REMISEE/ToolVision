...
class PlannerClientRequest(PipelineBaseModel):
    ...
    def _validate_required_fields(self) -> "PlannerClientRequest":
        ...
        if self.answer_instruction is not None and not str(self.answer_instruction).strip():
            raise ValueError("PlannerClientRequest.answer_instruction must not be empty when provided.")
        ...
        if self.requested_suggestion_count is not None and not 0 <= self.requested_suggestion_count <= 3:
            raise ValueError("PlannerClientRequest.requested_suggestion_count must be between 0 and 3.")
        if self.must_answer_now and self.requested_suggestion_count not in (None, 0):
            raise ValueError(
                "PlannerClientRequest.requested_suggestion_count must be 0 or null when must_answer_now=True."
            )
        return self
...
class JudgeClientRequest(PipelineBaseModel):
    ...
    def _validate_required_fields(self) -> "JudgeClientRequest":
        ...
        if self.answer_instruction is not None and not str(self.answer_instruction).strip():
            raise ValueError("JudgeClientRequest.answer_instruction must not be empty when provided.")
        if not self.messages:
