...
class RootSample(PipelineBaseModel):
    ...
    def _validate_images(self) -> "RootSample":
        if not self.question.strip():
            raise ValueError("RootSample.question must not be empty.")
        if not self.images:
        ...
        if self.answer_instruction is not None and not str(self.answer_instruction).strip():
            raise ValueError("RootSample.answer_instruction must not be empty when provided.")
        if isinstance(self.answer, list):
            if not self.answer:
                raise ValueError("RootSample.answer must not be an empty list.")
            normalized_answers: list[str] = []
            for item in self.answer:
                text = str(item).strip()
                if not text:
                    raise ValueError("RootSample.answer list items must not be empty.")
                normalized_answers.append(text)
            object.__setattr__(self, "answer", normalized_answers)
        elif self.answer is not None:
            text = str(self.answer).strip()
            if not text:
                raise ValueError("RootSample.answer must not be empty when provided.")
            object.__setattr__(self, "answer", text)
        return self
...
class PlannerStepSpec(PipelineBaseModel):
    step_id: str
    step_goal: str
    input_image_index: int = Field(ge=0)
