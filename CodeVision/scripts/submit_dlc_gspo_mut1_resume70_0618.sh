...
# Resume the original MUT v1 run from the clean global_step_70 checkpoint.
# Keep reward/data unchanged.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"

TOOL_IP="${TOOL_IP:-}"
TOOL_REPLICA_INDEX="${TOOL_REPLICA_INDEX:-0}"
...
export JOB_NAME="${JOB_NAME:-cv-mut1-resume70}"
export EXP_NAME="${EXP_NAME:-mutv1_resume70_0618}"
export RESUME_MODE="${RESUME_MODE:-resume_path}"
export RESUME_FROM_PATH="${RESUME_FROM_PATH:-./saves/ToolVisionRL/mutv1_a/global_step_70}"
