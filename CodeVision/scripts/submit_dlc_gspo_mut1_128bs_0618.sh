...
# MUT v1 from v03 SFT, batch-size ablation.
# Only train batch size changes by default; reward/data stay unchanged.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"

TOOL_IP="${TOOL_IP:-}"
TOOL_REPLICA_INDEX="${TOOL_REPLICA_INDEX:-1}"
...
export JOB_NAME="${JOB_NAME:-cv-mut1-128bs}"
export EXP_NAME="${EXP_NAME:-mutv1_128bs_0618}"
export RESUME_MODE="${RESUME_MODE:-disable}"
export RESUME_FROM_PATH="${RESUME_FROM_PATH:-null}"

export TRAIN_BSZ="${TRAIN_BSZ:-128}"
