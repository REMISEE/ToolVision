...
EXP_NAME="${EXP_NAME:-qwen3vl8b_gspo}"
SAVE_DIR="${SAVE_DIR:-}"
...
# ------------------------------------------------------------
# GSPO here means using GRPO outcome advantages with the GSPO policy loss.
adv_estimator="grpo"
...
tool_reward_tool_error_weight=0.2
tool_reward_invalid_call_weight=0.2
...
RAY_JOB_SUBMIT_EXTRA_ARGS=()
if [[ "${RAY_JOB_NO_WAIT:-0}" == "1" ]]; then
    RAY_JOB_SUBMIT_EXTRA_ARGS+=(--no-wait)
fi

ray job submit "${RAY_JOB_SUBMIT_EXTRA_ARGS[@]}" \
    --address="${RAY_ADDRESS}" \
    --working-dir $(pwd) \
