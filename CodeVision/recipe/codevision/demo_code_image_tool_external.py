...
3) GroundedSAM2 helper:
   ...
   - _call_dino_crop
   - _call_blur_bg
4) CountGD helper: _call_count_assist
5) 兼容别名 _call_focus（等价于 _call_ground_box）
...
"""
    ...
    # Case 7: CountGD 计数
    count_code = f"""
# CountGD: 返回计数可视化结果与结构化 count 元数据
count_res = _call_count_assist(
    {args.count_prompt!r},
    confidence_thresh={args.count_confidence_thresh},
    visualize={args.count_visualize!r},
)
print(count_res["text"])
print("predicted_count=", count_res["meta"].get("count"))
result = count_res["image"]
"""

    # Case 8: grounding + depth
    ground_depth_code = f"""
# Ground depth: 先找目标，再量该检测区域深度
depth_res = _call_ground_depth(
    {args.focus_prompt!r},
    detection_index=0,
    box_threshold={args.box_threshold},
    text_threshold={args.text_threshold},
...
print(depth_res["text"])
result = depth_res["image"]
"""
    ...
    return [
        ...
        {"name": "count_assist", "description": "CountGD 计数辅助", "code": count_code},
        {"name": "ground_depth", "description": "Grounding + Depth 区域深度", "code": ground_depth_code},
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--depth-base-url", type=str, default="http://127.0.0.1:8082")
    parser.add_argument("--countgd-base-url", type=str, default="http://127.0.0.1:8083")
    ...
    # CountGD 参数
    parser.add_argument("--count-prompt", type=str, default="car")
    parser.add_argument("--count-confidence-thresh", type=float, default=0.23)
    parser.add_argument("--count-visualize", type=str, default="heatmap", choices=["heatmap", "boxes"])
