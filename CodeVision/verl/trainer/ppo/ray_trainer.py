...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Store images and tool details for later logging
            ntb = test_output_gen_batch.non_tensor_batch
            imgs = ntb.get("merged_images", None)
            if imgs is None:
                val_images_agg.extend([None] * len(output_texts))
            else:
                try:
                    val_images_agg.extend(list(imgs))
                except Exception:
                    val_images_agg.extend([None] * len(output_texts))

            details = ntb.get("tool_reward_detail", None)
            if details is None:
                val_details_agg.extend([None] * len(output_texts))
            else:
                try:
                    val_details_agg.extend(list(details))
                except Exception:
                    val_details_agg.extend([None] * len(output_texts))
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (
                        ...
                        metric_sec = "val-aux"
                    pfx = f"val-{data_source}/{var_name}"
