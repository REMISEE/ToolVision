...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Reward component metrics (from agent loop tool-aware reward)
    # These are per-sample arrays attached in AgentLoopManager._post_aggregate_tool_rewards
    # We aggregate them into mean/max/min under the reward/ section for logging
    ...
    reward_component_keys = [
        ...
        "R_emerge",
        "R_total",
        # rnec_only / rnec_with_clean / simple_penalty diagnostics.
        # Keys absent from a given mode's components dict are silently skipped
        # by the `if key in batch.non_tensor_batch` guard below.
        "R_nec",
        "R_clean_tool_baseline",
        "R_clean_tool_success",
        "R_clean_tool",
        "P_overuse",
        "P_bad_tool",
        "P_tool_error",
    ...
    for key in reward_component_keys:
        try:
            if key in batch.non_tensor_batch:
                ...
                # Normalize to 1D numpy array of length bsz
                try:
                    import numpy as _np  # local import to avoid linter complaining about alias shadowing
                ...
                # Aggregate
                metrics[f"reward/{key}"] = float(vals.mean())
        except Exception:
            # Best-effort; continue on any unexpected shape/type
            continue
