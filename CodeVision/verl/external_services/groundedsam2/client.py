...
class GroundedSAM2HTTPClient:
    ...
    def _request_json(self, payload: dict[str, Any]) -> dict[str, Any]:
        resp = requests.post(self.infer_url, json=payload, timeout=self.request_timeout)
        if resp.status_code != 200:
            raise RuntimeError(f"GroundedSAM HTTP {resp.status_code}: {resp.text[:800]}")
        data = resp.json()
        if data.get("errorCode", 0) != 0:
            raise RuntimeError(
                f"GroundedSAM service error: code={data.get('errorCode')} msg={data.get('errorMsg')}"
            )
        result = data.get("result")
        if not isinstance(result, dict):
            raise RuntimeError(f"GroundedSAM response missing result object: {str(data)[:800]}")
        return result
    ...
    def _build_payload(self, image: Image.Image, kwargs: dict[str, Any]) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            "operation": str(kwargs.get("_operation", kwargs.get("operation", "box"))),
        }
        field_names = [
            "text_prompt",
            "box_threshold",
    ...
    def infer(self, image: Image.Image, kwargs: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(image, Image.Image):
            raise RuntimeError("GroundedSAM2HTTPClient expects PIL image input.")
