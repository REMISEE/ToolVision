...
from ..external_services.countgd.client import CountGDHTTPClient
from ..external_services.depth.client import DepthHTTPClient
...
SUCCESS_FOLLOWUP_TEXT = (
    ...
    "If additional tools are required, call them inside the <tool_call></tool_call> tags. "
...
class CountGDServiceAdapter(BaseExternalModelAdapter):
    """HTTP adapter for CountGD counting service endpoints."""

    def __init__(self, config: dict[str, Any]):
        super().__init__(config)
        self.client: Optional[CountGDHTTPClient] = None

    def initialize(self):
        if self._initialized:
            return
        base_url = str(self.config.get("base_url") or "http://127.0.0.1:8083")
        self.client = CountGDHTTPClient(
            base_url=base_url,
            request_timeout=int(self.config.get("request_timeout", 180)),
        )
        self._initialized = True
    ...
    def infer(self, image: Any, kwargs: dict[str, Any]) -> dict[str, Any]:
        self.initialize()
        if self.client is None:
            raise RuntimeError("CountGD service client is not initialized.")
        if not isinstance(image, Image.Image):
            raise RuntimeError("CountGD service adapter expects PIL image input.")
        return self.client.infer(image, kwargs or {})


class DepthServiceAdapter(BaseExternalModelAdapter):
    """HTTP adapter for Depth Pro service endpoints."""

    def __init__(self, config: dict[str, Any]):
        super().__init__(config)
        self.client: Optional[DepthHTTPClient] = None

    def initialize(self):
        if self._initialized:
            return
        base_url = str(self.config.get("base_url") or "http://127.0.0.1:8082")
        self.client = DepthHTTPClient(
            base_url=base_url,
            request_timeout=int(self.config.get("request_timeout", 180)),
        )
        self._initialized = True
    ...
    def infer(self, image: Any, kwargs: dict[str, Any]) -> dict[str, Any]:
        self.initialize()
        if self.client is None:
            raise RuntimeError("Depth service client is not initialized.")
        if not isinstance(image, Image.Image):
            raise RuntimeError("Depth service adapter expects PIL image input.")
        return self.client.infer(image, kwargs or {})
...
class CodeImageTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        self.service_adapter_registry: dict[str, type[BaseExternalModelAdapter]] = {
            ...
            "depth": DepthServiceAdapter,
            "countgd": CountGDServiceAdapter,
            "count": CountGDServiceAdapter,
        }
    ...
    def _get_service_model_config(self, model_name: str) -> dict[str, Any]:
        ...
        if normalized in {"countgd", "count"}:
            candidates.extend(["countgd", "count"])
    ...
    def _create_safe_globals(self, image, images: list[Any], image_index: int) -> dict:
        ...
        def _call_draw_point(
            ...
            def _impl() -> dict[str, Any]:
                selected = _select_image(target_index=image_index, image_obj=image_obj)
                if not hasattr(selected, "size"):
                    raise RuntimeError("Selected image does not provide size information.")
                img_width, img_height = selected.size
                px = int(round(float(x)))
                py = int(round(float(y)))
                if px < 0 or py < 0 or px >= int(img_width) or py >= int(img_height):
                    raise RuntimeError(f"Point ({px}, {py}) is outside image size {img_width}x{img_height}.")
                r = max(1, int(radius))
                out = selected.copy()
                draw_obj = ImageDraw.Draw(out)
                draw_obj.ellipse(
                ...
                if label is not None:
                    text_fill = label_fill or outline
                    draw_obj.text((min(px + r + 2, max(0, img_width - 1)), max(0, py - r - 12)), str(label), fill=text_fill)
                return _build_local_helper_result(
                    output_image=out,
                    text=f"Point drawn at [{px}, {py}].",
                    meta={
                        "model": "local_geometry",
                        "operation": "draw_point",
                        "point": [px, py],
                        "radius": r,
                        "outline": outline,
                        "fill": fill,
            ...
            return _run_helper("_call_draw_point", _impl)
        ...
        def _call_draw_line(
            ...
            def _impl() -> dict[str, Any]:
                ...
                draw_obj.line((p1[0], p1[1], p2[0], p2[1]), fill=fill, width=line_width)
                if label is not None:
                    text_fill = label_fill or fill
                    mid_x = min(max(0, int(round((p1[0] + p2[0]) / 2))), max(0, int(img_width) - 1))
                    mid_y = min(max(0, int(round((p1[1] + p2[1]) / 2))), max(0, int(img_height) - 1))
                    draw_obj.text((mid_x + 2, max(0, mid_y - 12)), str(label), fill=text_fill)
                return _build_local_helper_result(
                    output_image=out,
                    text=f"Line drawn from {p1} to {p2}.",
                    meta={
                        "model": "local_geometry",
                        "operation": "draw_line",
            ...
            return _run_helper("_call_draw_line", _impl)
        ...
        # CountGD helper：根据文本类别对实例进行计数，并返回可视化结果。
        def _call_count_assist(
            text_prompt: str,
            image_index: Optional[int] = None,
            image_obj: Optional[Any] = None,
            confidence_thresh: float = 0.23,
            visualize: str = "heatmap",
            **kwargs,
        ):
            call_kwargs = {
                "text_prompt": text_prompt,
        ...
        # Depth helper：对显式坐标框做区域深度统计并返回整图可视化。
        def _call_manual_depth(
            x1: float,
            y1: float,
            x2: float,
            y2: float,
            ...
            image_obj: Optional[Any] = None,
            stat: str = "median",
            vis_mode: str = "overlay",
            padding: int = 0,
            label: Optional[str] = None,
            **kwargs,
        ):
            call_kwargs = {
                "_operation": "manual_depth",
                "x1": x1,
                "y1": y1,
                "x2": x2,
                "y2": y2,
        ...
        # Depth helper：先 grounding 找框，再对该检测框做区域深度统计。
        def _call_ground_depth(
            text_prompt: str,
            image_index: Optional[int] = None,
            image_obj: Optional[Any] = None,
            detection_index: int = 0,
        ...
        safe_globals.update({
            ...
            '_call_draw_point': _call_draw_point,
            '_call_draw_line': _call_draw_line,
            ...
            '_call_count_assist': _call_count_assist,
            '_call_manual_depth': _call_manual_depth,
            '_call_ground_depth': _call_ground_depth,
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if not code or not isinstance(code, str):
            return (
                ToolResponse(text="Error: 'code' parameter is missing or not a string. Please keep thinking step-by-step inside the <think></think> tags to determine the next action. If additional tools are required, call them inside the <tool_call></tool_call> tags. Otherwise, provide your final answer within the <answer></answer> tags."),
                -0.05,
        ...
        if not isinstance(image_index, int) or image_index < 0:
            return (
                ToolResponse(text="Error: 'image_index' must be a non-negative integer. Please keep thinking step-by-step inside the <think></think> tags to determine the next action. If additional tools are required, call them inside the <tool_call></tool_call> tags. Otherwise, provide your final answer within the <answer></answer> tags."),
                -0.05,
        ...
        if image_index >= len(images):
            return (
                ToolResponse(text=f"Error: image_index {image_index} is out of range. Available images: 0 to {len(images)-1}. Please keep thinking step-by-step inside the <think></think> tags to determine the next action. If additional tools are required, call them inside the <tool_call></tool_call> tags. Otherwise, provide your final answer within the <answer></answer> tags."),
                -0.05,
        ...
        try:
            ...
            if result is None:
                return (
                    ToolResponse(text=f"Error: {message}. Please keep thinking step-by-step inside the <think></think> tags to determine the next action. If additional tools are required, call them inside the <tool_call></tool_call> tags. Otherwise, provide your final answer within the <answer></answer> tags."),
                    -0.05,
                    {
                        "success": False,
            ...
            output_image_index = len(images) - 1
            instance_data["last_input_image_index"] = image_index
            instance_data["last_output_image_index"] = output_image_index
            size_prefix = ""
            if hasattr(output_image, "size"):
                out_w, out_h = output_image.size
                size_prefix = f"Image size = {out_w}x{out_h} pixels.\n\n"

            response_text = helper_text or message
            if response_text:
                response_text = f"{size_prefix}{response_text}\n\n{SUCCESS_FOLLOWUP_TEXT}"
            else:
                response_text = f"{size_prefix}{SUCCESS_FOLLOWUP_TEXT}"
            ...
            return (
                ToolResponse(text=f"Error processing image {image_index}: {str(e)}. Please keep thinking step-by-step inside the <think></think> tags to determine the next action. If additional tools are required, call them inside the <tool_call></tool_call> tags. Otherwise, provide your final answer within the <answer></answer> tags."),
                -0.05,
                {
                    "success": False,
