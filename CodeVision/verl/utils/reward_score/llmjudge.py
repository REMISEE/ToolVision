...
class LLMJudgeClient:
    ...
    def __init__(self, base_url: str, model_name: Optional[str] = None, 
        ...
        self.trust_env = _env_flag("LLM_JUDGE_TRUST_ENV") if trust_env is None else trust_env
        self.extra_body = {}
        if os.getenv("LLM_JUDGE_ENABLE_THINKING") is not None:
            self.extra_body["enable_thinking"] = _env_flag("LLM_JUDGE_ENABLE_THINKING")
        if os.getenv("LLM_JUDGE_EXTRA_BODY"):
            try:
                self.extra_body.update(json.loads(os.environ["LLM_JUDGE_EXTRA_BODY"]))
            except json.JSONDecodeError as exc:
                logger.warning(f"Invalid LLM_JUDGE_EXTRA_BODY JSON: {exc}")
    ...
    def _initialize_client(self, api_key: str, model_name: Optional[str], timeout: int):
        ...
        try:
            self.client = OpenAI(
                ...
                http_client=DefaultHttpxClient(trust_env=self.trust_env, timeout=timeout),
            )
