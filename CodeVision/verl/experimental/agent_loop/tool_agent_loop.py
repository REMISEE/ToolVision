...
def _normalize_helper_tool_name(name: Any) -> str:
    ...
    if name.startswith("_call_"):
        name = name[len("_call_") :]
    return name


def _helper_tool_names_from_response(tool_response: ToolResponse) -> list[str]:
    meta = getattr(tool_response, "meta", None)
    if not isinstance(meta, dict):
        return []

    tool_metrics = meta.get("_tool_metrics")
    if not isinstance(tool_metrics, dict):
        return []

    observed_calls = tool_metrics.get("observed_helper_calls")
    ...
    for item in observed_calls:
        if not isinstance(item, dict):
            continue
        name = _normalize_helper_tool_name(item.get("name"))
        if name:
            names.append(name)
    return names
...
class ToolAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.max_response_tokens_per_turn = config.actor_rollout_ref.rollout.multi_turn.get(
            "max_response_tokens_per_turn", None
        )
    ...
    async def _handle_generating_state(self, agent_data: AgentData, sampling_params: dict[str, Any]) -> AgentState:
        ...
        with simple_timer("generate_sequences", agent_data.metrics):
            ...
            max_response_tokens_per_turn = self.max_response_tokens_per_turn
            if max_response_tokens_per_turn is not None:
                try:
                    remaining_response_tokens = min(remaining_response_tokens, int(max_response_tokens_per_turn))
                except Exception:
                    pass
            turn_sampling_params["max_tokens"] = max(1, remaining_response_tokens)
    ...
    async def _call_tool(self, tool_call: FunctionCall, tools_kwargs: dict[str, Any]) -> ToolResponse:
        ...
        response_meta = getattr(tool_execution_response, "meta", None)
        if isinstance(response_meta, dict):
            response_meta = dict(response_meta)
        else:
            response_meta = {}
        if isinstance(tool_metrics, dict):
            response_meta["_tool_metrics"] = _to_jsonable(tool_metrics)
        if response_meta:
            tool_response_kwargs["meta"] = response_meta
