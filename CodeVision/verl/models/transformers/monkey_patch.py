...
def patch_forward_with_backends(
    ...
    elif model.config.model_type in ["qwen3_vl"]:
...
def apply_monkey_patch(
    ...
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        ...
        # Step 1: patch model to support image-text mixed data (Qwen3-VL)
    ...
    elif model.config.model_type in ["qwen3_vl_moe"]:
        # Step 1: patch model to support image-text mixed data (Qwen3-VL-MoE)
        try:
            ...
            print(f"Monkey patch {model.__class__.__name__} model forward (MoE)")

            if ulysses_sp_size > 1:
                patch_vlm_for_ulysses_input_slicing(Qwen3VLMoETextModel)
        except Exception as e:
            # Fallback: patch the instance class only to avoid breaking other models
            try:
                from verl.models.transformers.qwen3_vl import forward_with_normal_backend

                model.__class__.forward = forward_with_normal_backend
                print(
                    f"Fallback monkey patch forward for {model.__class__.__name__} (MoE). Import error: {e}"
                )
            except Exception:
                print(
                    f"Warning: Failed to monkey patch Qwen3-VL-MoE. The model will run with its default forward."
                )
